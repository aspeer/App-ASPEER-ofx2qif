#!/bin/env perl
#
#  Convert Westpac OFX file to QIF
#
package main;
...
sub getopt {
    ...
    #  Get in and outfiles from remaining arguments
    #
    $opt{'infile'} = $argv_ar->[0] ||
        die "usage: $Script infile <outfile>";
    $opt{'outfile'} = $argv_ar->[1] || +OUTFILE;
...
sub main {
    ...
    #
    my ($ofx_account_type, $ofx_qif_header, $ofx_lookup_trxn_ar, $ofx_lookup_id_ar, $qif_header);
    my %qif_lookup=(
    ...
    #
    my $ofx_account_id=$ofx_hr->{'ofx'}{$ofx_account_type};
...
B<ofx2qif.pl> B<[OPTIONS]> B<INFILE> B<<OUTFILE>>
